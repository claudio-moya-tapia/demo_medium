...
class DocumentoController extends Controller {
    ...
    public function actionCreate() {
        ...
        if ($UploadFile = CUploadedFile::getInstanceByName('Filedata')) {
            ...
            $formatName = date('Y_m_d_H_i_s_') . '_' . rand(100000, 999999) . '.' . $UploadFile->getExtensionName();
            $UploadFile->saveAs($dir . $formatName);

            $httpOriginal = str_replace(Yii::getPathOfAlias('webroot'), '', $dir . $formatName);
            
            $Documento = new Documento();
            $Documento->tipo_documento_id = 1;
            $Documento->fecha_creacion = $DateTime->format('Y-m-d H:i:s');
            $Documento->nombre = $UploadFile->getName();
            $Documento->mimetype = $UploadFile->getType();
